...
<div class="wrap">
  ...
  <div class="cfg" id="cfg" hidden>
    <h4>Filters</h4>
    <div class="filters" style="display:flex;flex-wrap:wrap;gap:16px;align-items:center;margin:6px 0 4px;font-size:14px">
      <label style="display:flex;gap:6px;align-items:center;cursor:pointer"><input type="checkbox" id="fOem"> OEM parts</label>
      <label style="display:flex;gap:6px;align-items:center;cursor:pointer"><input type="checkbox" id="fAft"> Aftermarket parts</label>
      <label style="display:flex;gap:6px;align-items:center;cursor:pointer"><input type="checkbox" id="fSalv"> Salvage vehicles</label>
      <label style="display:flex;gap:6px;align-items:center">Country
        <select id="fCountry" style="background:var(--bg);color:var(--ink);border:1px solid var(--line);border-radius:6px;padding:5px 8px;font-family:inherit">
...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  function render() {
    ...
    // Group listings by their section, then show config categories first followed by
    // ANY other section that actually has listings — so no result is ever hidden.
    const bySec = {}; state.listings.forEach((l) => { const s = l.section || 'Results'; (bySec[s] = bySec[s] || []).push(l); });
  ...
  async function vote(l, value) {
    try {
      ...
      if (value === -1) { state.listings = state.listings.filter((x) => x.url !== l.url); }
      render();
      PH.toast(value === 0 ? 'Feedback cleared' : (value > 0 ? 'Marked good' : 'Removed — won\'t come back'));
